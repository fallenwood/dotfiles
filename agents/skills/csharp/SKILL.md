...
0. Always respect `.editorconfig` if present
1. Always add prefix `this` when accessing instance members
2. Always add brackets for single-line `if`, `for`, `while`, and `using` statements
3. Always put namespace declarations on top of using directives
4. Use Primary Constructors if possible
5. Avoid reflection and dynamic code generation unless absolutely necessary
6. Prefer `var` for local variable declarations when the type is obvious from the right-hand side
7. Avoid Ternary conditional operator `? :` unless the expression is simple and fits on one line
8. Ask user to choose to use Central Package Management or not
